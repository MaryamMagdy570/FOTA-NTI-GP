	...
	if (SessionHandler->SessionID != 0 && SessionHandler->SessionID < 8)
	{
		...
		switch (SessionHandler->Session_Protocol)
		{
			...
			case SESSION_SPI:
				...
				break;
			case SESSION_I2C:
				_I2Cx_Init((SessionHandler->pSessionHandler), SessionHandler->SessionID);
				break;
			case SESSION_CAN:
		...
		switch (SessionHandler->Session_Protocol)
			{
				...
				case SESSION_SPI:
					...
					break;
				case SESSION_I2C:
					_I2Cx_DeInit(SessionHandler->pSessionHandler);
					break;
				case SESSION_CAN:
					/*		CAN initialization function*/
					break;
	...
	if (sessID != 0 && sessID < 8)
	{
		...
		switch (ARR_SESSIONS[sessID]->Session_Protocol)
		{
			case SESSION_UART:
				for ( uint8_t var = 0; var < Size; ++var)
				{
					if (pdata[var] == Last_element) {
						HAL_UART_Transmit(((UART_HANDLER*)(ARR_SESSIONS[sessID]->pSessionHandler))->HUSARTx, pdata, 1, 10);
						break;
					}
					else {
						HAL_UART_Transmit(((UART_HANDLER*)(ARR_SESSIONS[sessID]->pSessionHandler))->HUSARTx, pdata, 1, 10);
			...
			case SESSION_SPI:
				...
				{
					if (pdata[var] == Last_element) {
						HAL_SPI_Transmit(((SPI_HANDLER*)(ARR_SESSIONS[sessID]->pSessionHandler))->HSPIx, pdata, 1, 10);
						break;
					}
					else {
						HAL_SPI_Transmit(((SPI_HANDLER*)(ARR_SESSIONS[sessID]->pSessionHandler))->HSPIx, pdata, 1, 10);
