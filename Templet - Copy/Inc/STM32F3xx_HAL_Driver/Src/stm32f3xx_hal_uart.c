...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_UART_Transmit(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size, uint32_t Timeout , const uint8_t Last_element)
{
  ...
  if (huart->gState == HAL_UART_STATE_READY)
  {
    ...
    while (huart->TxXferCount > 0U)
    {
      ...
      if (pdata8bits == NULL)
      {
        ...
        if ( *pdata16bits == (uint16_t)Last_element)
        {
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_UART_Receive(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, uint32_t Timeout, const uint8_t Last_element)
{
  uint8_t  *pdata8bits;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_UART_Transmit_IT(UART_HandleTypeDef *huart, const uint8_t *pData, uint16_t Size, const uint8_t Last_element)
{
  /* Check that a Tx process is not already ongoing */
  if (huart->gState == HAL_UART_STATE_READY)
  {
    ...
    huart->TxXferSize  		= Size;
    huart->TxXferCount 		= Size;
    huart->TxLastelement 	= Last_element;
    huart->TxISR      		= NULL;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef HAL_UART_Receive_IT(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, const uint8_t Last_element)
{
  /* Check that a Rx process is not already ongoing */
  if (huart->RxState == HAL_UART_STATE_READY)
  {
    ...
    return (UART_Start_Receive_IT(huart, pData, Size ,Last_element));
  }
  else
  {
    return HAL_BUSY;
...
/**
  ...
  * @retval HAL status
  */
HAL_StatusTypeDef UART_Start_Receive_IT(UART_HandleTypeDef *huart, uint8_t *pData, uint16_t Size, const uint8_t Last_element)
{
  huart->pRxBuffPtr  	= pData;
  huart->RxXferSize  	= Size;
  huart->RxXferCount 	= Size;
  huart->RxLastelement	= Last_element;
  huart->RxISR       	= NULL;
...
static void UART_TxISR_8BIT(UART_HandleTypeDef *huart)
{
  ...
  if (huart->gState == HAL_UART_STATE_BUSY_TX)
  {
    ...
    else
    {
      ...
      // Check the last element.
      if ( *huart->pTxBuffPtr == huart->TxLastelement)
      {
         (huart -> TxXferSize) = huart -> TxXferCount;
         huart ->TxXferCount = 0;
...
static void UART_RxISR_8BIT(UART_HandleTypeDef *huart)
{
  ...
  if (huart->RxState == HAL_UART_STATE_BUSY_RX)
  {
    ...
    *huart->pRxBuffPtr = (uint8_t)(uhdata & (uint8_t)uhMask);
    if (huart->RxLastelement == *huart->pRxBuffPtr)
        {
          /* Disable the UART Parity Error Interrupt and RXNE interrupts */
          ATOMIC_CLEAR_BIT(huart->Instance->CR1, (USART_CR1_RXNEIE | USART_CR1_PEIE));
